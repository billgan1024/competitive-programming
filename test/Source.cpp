...
void upd(int l, int r, int a, int b, int p, int v) {
    //update seg tree of dp[]+c[] indicating that all segments in that range should have their values updated to 
...
int main() {
    ...
    for(int d = 1; d <= ceil((double)N/K); d++) {
        ...
        for(int i = l; i <= r; i++) {
            ...
            //calculate dp and update the seg tree of dp
            ...
            upd(i, i, 0, maxN-1, 1, i+1 > r ? 0 : st_query(i+1, r));
        }
        ...
        for(int i = l; i <= r; i++) c.push_back({i+1 > r ? 0 : st_query(i+1, r), i});
    }
}
