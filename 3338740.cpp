...
int n, m, ans; map<int, map<int, int>> ds; unordered_map<int, int> posID, negID;
...
int main() {
    ...
    for(auto p : ds) {
        ...
        while(cur < m && b[cur].first < p.first) {
            if(pos.order_of_key({b[cur].second, INT_MAX}) - neg.order_of_key({b[cur].second, INT_MAX}) > 0) ans++;
            cur++;
        }
        //update line sweeps
        for(pii q : p.second) {
            ...
            for(int i = 0; i < abs(q.second); i++) {
                if(q.second > 0) {
                    posID[q.first]++;
                    int c = posID[q.first];
                    pos.insert({q.first, c});
                } else {
                    negID[q.first]++; int c = negID[q.first]; neg.insert({q.first, c});
                }
            }
        }
    }
