...
//otherwise, add this number to the shortest path total for THIS EDGE:
//(# of shortest paths from source to e.u) * (# of shortest paths from e.v to any other node)
...
int dfs(int c)
{
    //base: return 0 or the pre-calculated from[c] value
    ...
    for(edge e : adj[c])
    {
        ...
        if(dis[c] + e.wt == dis[e.to]) f = (f + dfs(e.to) + 1) % mod;
    }
    return from[c] = f;
}
