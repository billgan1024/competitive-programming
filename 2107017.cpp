...
//technically bit operations are constant exdee
...
int f(ui b, int i)
{
    ...
    if((b & last[i]) == 0) dp[b][i] = min(dp[b][i], f(b, i-1));
    return dp[b][i];
}
...
int main()
{
    ...
    while(true)
    {
        ...
        if(dp[mask][cur] != dp[mask][cur-1] || (mask & last[cur]) > 0)
        {
            print(cur+1, sp); mask = (mask ^ dist[cur]) & mask;
        }
        cur--;
    }
}