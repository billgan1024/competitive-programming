...
int main()
{
    //dp[i][j][0] = min distance travelled to visit pies i to j, ending at pie i
    //dp[i][j][1] = min distance travelled to visit pies i to j, ending at pie j
    //note that i and j will enclose the pie at zero (also it is -1 if impossible)
    cin.sync_with_stdio(0); cin.tie(0); scan(n);
    for(int i = 0; i < n; i++) scan(p[i].x, p[i].d);
    sort(p, p+n); s[0] = p[0].d; for(int i = 1; i < n; i++) s[i] = s[i-1] + p[i].d;
    int o = binarySearch(0, n-1);
    dp[o][o][0] = dp[o][o][1] = 0; ans = sum(o, o);
