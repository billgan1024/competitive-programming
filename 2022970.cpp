...
int n, m, len = INT_MAX, d1[701], d2[702], dis1[701], dis2[701]; bool s[701]; vector<pii> adj[701];
ll ans, w1[701], w2[701], ways1[701], ways2[701];
...
int main()
{
    ...
    scan(m);
    for(int i = 1, s0; i <= m; i++)
    {
        scan(s0); s[s0] = true;
    }
    dijkstra1(1, d1, w1); dijkstra1(n, d2, w2);
    dijkstra2(1, dis1, ways1); dijkstra2(n, dis2, ways2);
    for(int i = 1; i < n; i++)
    {
        if(s[i])
        {
            if(d1[i] == dis1[i])
            {
                if(len > d1[i] + dis2[i])
                {
                    len = d1[i] + dis2[i]; ans = (w1[i]*ways2[i]) % mod;
                }
                else if(len == d1[i] + dis2[i]) ans = (ans + w1[i]*ways2[i] % mod) % mod;
            }
            /*else if(d2[i] == dis2[i])
            {
                if(len > dis1[i] + d2[i])
                {
                    len = dis1[i] + d2[i]; ans = (ways1[i]*w2[i]) % mod;
                }
                else if(len == dis1[i] + d2[i]) ans = (ans + ways1[i]*w2[i] % mod) % mod;
            }*/
        }
    }
    print(len, sp, ans);
