...
int n, m, q, p[1001]; edge edges[5001]; vector<edge> order; vector<pii> adj[1001];
...
void tree()
{
    ...
    sort(order.begin(), order.end(), cmp); for(int i = 1; i <= n; i++) p[i] = i; int cnt = n;
    for(edge e : order)
    {
        if(merge(e.u, e.v))
        {
