...
public class ContestOfficial3 {
	...
	{
		Queue<Integer> queue = new LinkedList<Integer>();
		dis2 = new int[n]; prev = new int[n];
		vis = new boolean[n];
		
		queue.add(start); vis[start] = true; prev[start] = -1;
		while(!queue.isEmpty())
		{
			int cur = queue.remove();
			for(Edge e : adj[cur])
