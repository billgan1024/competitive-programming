...
int main() {
    ...
    while(!p.empty()) {
        pii cur = p.top(); p.pop(); int v = g[cur.second];
        if(dis[cur.second] < cur.first) continue;
        for(int i = 1; i <= k; i++) {
            if(!a[g[cur.second]][i]) continue;
            //find up to two nodes in the group which can be reached from this group, or jump to the goal directly
            if(b[i].find(n) != b[i].end()) dis[n] = min(dis[n], cur.first + abs(n-cur.second));
