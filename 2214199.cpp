...
int main()
{
    ...
    cin.sync_with_stdio(0); cin.tie(0); sc(n, m, k, s); ms(dis, INF); ms(dp, INF); ms(best, INF);
    for(int i = 0; i < m; i++)
    {
    ...
    //initialize best[] (any best[i][j] where i < k will be 0 because best[0][j] = 0 and things after that 
    //are infinity)
    for(int i = 0; i < k; i++)
    {
