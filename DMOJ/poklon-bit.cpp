...
int main() {
    ...
    //any valid sequence of intervals is a subsequence of this array after we sort it like this
    //any valid sequence is one needs to have the right and left endpoints close in on each other
    //meaning that the left intervals must form an increasing sequence
    //the right intervals close in on each other as you progress down the list of intervals
    //so u don't need to consider the right intervals
